		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					...
					<el-input v-model="formSearch.chanpinmingcheng" placeholder="产品名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
				...
				<el-form-item class="list-item">
					...
					<el-input v-model="formSearch.dianpumingcheng" placeholder="店铺名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
				...
				<el-button class="list-add-btn" v-if="btnAuth('shumachanpin','新增')" type="primary" @click="add('/index/shumachanpinAdd')">
			...
			<div class="list">
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)">
						<div class="imgbox">
							<img @click.stop="imgPreView(item.chanpintupian)" v-if="item.chanpintupian && item.chanpintupian.substr(0,4)=='http'&&item.chanpintupian.split(',w').length>1" :src="item.chanpintupian" class="image" />
							<img @click.stop="imgPreView(item.chanpintupian.split(',')[0])" v-else-if="item.chanpintupian && item.chanpintupian.substr(0,4)=='http'" :src="item.chanpintupian.split(',')[0]" class="image" />
							<img @click.stop="imgPreView(baseUrl + (item.chanpintupian?item.chanpintupian.split(',')[0]:''))" v-else :src="baseUrl + (item.chanpintupian?item.chanpintupian.split(',')[0]:'')" class="image" />
						</div>
						<div class="infoBox">
...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/chanpinleixing/chanpinleixing').then(res => {
				if (res.data.code == 0) {
					this.chanpinleixingOptions = res.data.data;
					this.selectOptionsList.push({name:'产品类型',list:this.chanpinleixingOptions,tableName: 'chanpinleixing',check: -1})
				}
			});
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.chanpinleixing != '') searchWhere.chanpinleixing = this.formSearch.chanpinleixing;
				...
				this.$http.get(`shumachanpin/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/shumachanpinDetail', query: params});
			},

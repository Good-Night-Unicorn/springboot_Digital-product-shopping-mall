	...
	<!-- dianpumingcheng -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'≡'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/cuxiaohuodong?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('cuxiaohuodong','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('cuxiaohuodong','删除')" @click="delClick">删除</el-button>
						<el-button class="chatBtn" v-if="btnAuth('cuxiaohuodong','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			storeup(type) {
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'cuxiaohuodong', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			getThumbsupOrCrazilyStatus() {
				if(localStorage.getItem("frontToken")) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '21', refid: this.detail.id, tablename: 'cuxiaohuodong', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
							this.isThumbsupnum = true;
							this.thumbsupOrCrazilyInfo = res.data.data.list[0];
						}
					});
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '22', refid: this.detail.id, tablename: 'cuxiaohuodong', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
						if (res.data.code == 0 && res.data.data.list.length > 0) {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/cuxiaohuodong', query: params});
				}
			},
			// 下载
			download(file ){
			...
			getDiscussList(page) {
				this.$http.get('discusscuxiaohuodong/list', {params: {page, limit: this.pageSize, refid: this.detail.id,sort: 'istop',order: 'desc'}}).then(res => {
					if (res.data.code == 0) {
			...
			comzanClick(row){
				...
				if(!this.comzanChange(row)){
					...
					this.$http.post('discusscuxiaohuodong/update',row).then(rs=>{
					...
					if (valid) {
						this.$http.get('orders/list', {params: {page: 1, limit: 1, status: '已完成', goodid: this.detail.id, userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
							if (res.data.code == 0 && res.data.data.list.length == 0) {
								...
								this.$http.post('discusscuxiaohuodong/add', this.form).then(rs2 => {
									if (rs2.data.code == 0) {
			...
			getCartList() {
				this.$http.get('cart/list', {params: {userid: Number(localStorage.getItem('frontUserid')), tablename: 'cuxiaohuodong', goodid: this.detail.id}}).then(res => {
					if (res.data.code == 0) {
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此促销活动？') .then(_ => {
			...
			chatClick() {
				...
				if(localStorage.getItem('frontSessionTable')=='cuxiaohuodong') {

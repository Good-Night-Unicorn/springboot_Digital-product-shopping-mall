<template>
	<div :style='{"padding":"0 0 20px","margin":"0px auto","color":"#666","background":"#fff","width":"1200px","fontSize":"16px","position":"relative"}'>
		<div class="section-title" :style='{"padding":"0","margin":"0 1060px 20px 0","color":"#fff","borderRadius":"0px 0px 30px 30px","textAlign":"center","background":"#5FB6B8","display":"inline-block","width":"190px","fontSize":"22px","lineHeight":"60px","height":"60px"}'>购物车</div>
		...
		<el-table @selection-change="handleSelectionChange" :data="tableData" style="width: 100%">
			...
			<el-table-column label="价格">
				<template slot-scope="scope">
					<span :style='{"fontSize":"12px"}'>￥</span>{{ vip=='是'?scope.row.discountprice:scope.row.price }}
				</template>
			</el-table-column>
			...
			<el-table-column label="总价">
				<template slot-scope="scope">
					<span :style='{"fontSize":"12px"}'>￥</span>{{ vip=='是'?(scope.row.discountprice * scope.row.buynumber).toFixed(2):(scope.row.price * scope.row.buynumber).toFixed(2) }}
				</template>
			</el-table-column>
	...
	export default {
		...
		computed: {
			totalPrice() {
				...
				for(let x in this.selRows){
					if(this.vip=='是'){
						totalPrice += Number(this.selRows[x].discountprice) * Number(this.selRows[x].buynumber)
					}else{

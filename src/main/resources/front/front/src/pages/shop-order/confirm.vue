<template>
	<div :style='{"padding":"0 0 20px","margin":"0px auto","color":"#666","background":"#fff","width":"1200px","fontSize":"16px","position":"relative"}'>
		<div class="section-title" :style='{"padding":"0","margin":"0 1060px 20px 0","color":"#fff","borderRadius":"0px 0px 30px 30px","textAlign":"center","background":"#5FB6B8","display":"inline-block","width":"190px","fontSize":"22px","lineHeight":"60px","height":"60px"}'>确认{{totalPrice>0?'下单':'预订'}}</div>
		...
		<el-table
			...
			<el-table-column v-if="totalPrice>0"
				label="价格">
				<template slot-scope="scope">
					<span v-if="type == 2">{{ vip=='是'?scope.row.discountprice:scope.row.price }} 积分</span>
					<span v-else><span :style='{"fontSize":"12px"}'>￥</span>{{ vip=='是'?scope.row.discountprice:scope.row.price }}</span>
				</template>
			</el-table-column>
			...
			<el-table-column v-if="totalPrice>0"
				label="总价">
				<template slot-scope="scope">
					<span v-if="type == 2">{{ vip=='是'?(scope.row.discountprice * scope.row.buynumber):(scope.row.price * scope.row.buynumber) }} 积分</span>
...
export default {
	...
	methods: {
		changePrice(row){
			let price = 0
			if(this.vip=='是'){
				price = row.discountprice * row.buynumber
			}else {
			...
			for(let index in this.tableData){
				...
				// 获取商品详情信息
				 this.$http.get(`${item.tablename}/info/${item.goodid}`).then( res => {
					...
					if(item.tablename=='shumachanpin'){
						// 减少库存
					...
					// 更新库存信息
					 this.$http.post(`${item.tablename}/update`, data).then( res => {
						...
						let order = {
							...
							price: this.vip=='是'?item.discountprice:item.price,
	...
	computed: {
		totalPrice() {
			...
			this.tableData.forEach(item => {
				if(this.vip=='是'){
					totalPrice += item.discountprice * item.buynumber;
				}else{
